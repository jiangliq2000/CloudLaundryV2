    ...
    methods: {
      handleChangepwd() {
        ...
        if (that.form.newPwd !== that.form.confirmPwd) {
          ...
          API.changePassword(userInfo,para).then(function (result) {
            //this.loading = false;
            if (result && parseInt(result.errcode) === 0) {
              //修改成功
              that.$message.success({showClose: true, message: '修改成功, 5秒后将退出，请重新登录', duration: 3000});
              sessionStorage.removeItem('access-user');
              sessionStorage.removeItem('token');
              //that.$router.go('/login'); //用go刷新  
              setTimeout(function(){that.$router.go('/login');}, 5000)     
            } else if (result && parseInt(result.errcode) === 1) {
              that.$.error({showClose: true, message: "旧密码不对，请重试！", duration: 2000});
            }
            else {
